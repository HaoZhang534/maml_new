...
CUDA_VISIBLE_DEVICES=$GPU python main.py --datasource=$dataset --metatrain_iterations=$metatrain_iterations --meta_batch_size=$meta_batch_size --update_batch_size=$shot --update_lr=$update_lr --num_updates=$num_updates --logdir=$logdir  --reptile=$reptile --mode=$mode --train=$train --resume=$resume --poison_itr=$poison_itr --poison_lr=$poison_lr --meta_lr=$meta_lr --save_poison_ep=$save_poison_ep --median=$median --poison_dir=$poison_dir --noise_rate=$noise_rate
